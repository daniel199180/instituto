...
|---|---|
| **Sucursal** (`branches`) | Solo se puede eliminar si **todos sus cursos están en estado `cerrado`**. Si tiene algún curso en otro estado, la eliminación se bloquea y solo se permite desactivarla (`estado = inactiva`). |
...
| **Carrera** (`careers`) | Solo se puede eliminar si no tiene cursos asociados; de lo contrario se desactiva. |
...
- El instituto puede tener varias sucursales. Cada sucursal tiene su propio nombre, dirección y estado (activa/inactiva).
...
- **`deleteBranch(branchId)`** — Admin. Elimina la sucursal **solo si todos sus cursos están `cerrado`**; si no, bloquea la eliminación y sugiere desactivarla (`estado = inactiva`).
...
- **`deleteCareer(careerId)`** — Admin. Elimina la carrera **solo si no tiene cursos asociados**; si no, se desactiva.
